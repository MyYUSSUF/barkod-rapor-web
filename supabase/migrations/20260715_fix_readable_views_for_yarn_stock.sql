-- Refresh readable admin views after adding the yarn stock report permission.
...
select
  ...
  r.report_code,
  coalesce(
    nullif(r.report_name, ''),
    case r.report_code
      when 'RAR00037' then 'Iplik Stok Raporu'
      when 'RAR00026' then 'Fixing Raporu'
      when 'RAR00027' then 'Sevkiyat Raporu'
      else r.report_code
    end
  ) as report_name,
